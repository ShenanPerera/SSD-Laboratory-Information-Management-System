...
import { validatePassword } from '../../UtillFuntions/validationUtils';
...
const StaffForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const staff = {
      ...
      pw: DOMPurify.sanitize(pw),
    };

    const att = {
      name: DOMPurify.sanitize(name),
      Eid: DOMPurify.sanitize(Eid),
      attendance: DOMPurify.sanitize(attendance),
    };

    const sal = {
      Eid: DOMPurify.sanitize(Eid),
      post: DOMPurify.sanitize(post),
      salary: DOMPurify.sanitize(salary),
    };
  ...
  return (
    <form className="create" onSubmit={handleSubmit}>
      ...
      >
        ...
        <option value="Medical Lab Technologist">
